...
Vygenerováno: 2026-09-22 05:49 UTC · `npm run validate`
...
|---|---:|---:|
| výchozí pozice | +7 | 18 |
| bílý má navíc Rua (chybí r a8) | +746 | 19 |
| černý má navíc Rua (chybí R a1) | -708 | 16 |
| bílý má navíc Ma (chybí n b8) | +507 | 19 |
| bílý má navíc Khon (chybí s c8) | +376 | 19 |
| bílý má navíc Met (chybí m d8) | +246 | 20 |
| bílý má navíc Bia (chybí p d6) | +192 | 18 |
...
| strategy | central-push/c-pawn-knights | 12 | 0 | 0 | 0 | +38 (bílý na tahu) |
| strategy | central-push/e-pawn-kingside | 12 | 0 | 0 | 0 | -8 (bílý na tahu) |
...
| strategy | met-attack/met-g5-support | 12 | 0 | 0 | 0 | +115 (bílý na tahu) |
| strategy | met-attack/met-against-f5 | 12 | 0 | 0 | 0 | +15 (bílý na tahu) |
| strategy | met-attack/met-h4-diagonal | 12 | 0 | 0 | 0 | -84 (bílý na tahu) |
| strategy | met-attack/met-b4-diagonal | 12 | 0 | 0 | 0 | -84 (bílý na tahu) |
| strategy | rua-on-7th/open-a-file | 10 | 0 | 0 | 0 | +30 (bílý na tahu) |
| strategy | rua-on-7th/h-file-rook | 12 | 0 | 0 | 0 | +92 (bílý na tahu) |
| strategy | rua-on-7th/b-file-rook | 12 | 0 | 0 | 0 | +61 (bílý na tahu) |
| strategy | rua-on-7th/a-file-active-knights | 12 | 0 | 0 | 0 | +69 (bílý na tahu) |
| strategy | promo-prep/e-file-push | 10 | 0 | 0 | 0 | -15 (bílý na tahu) |
| strategy | promo-prep/d-pawn-e5-promo | 12 | 0 | 0 | 0 | -46 (bílý na tahu) |
| strategy | promo-prep/b-pawn-promo | 12 | 0 | 0 | 0 | -68 (bílý na tahu) |
| strategy | promo-prep/c-pawn-via-b | 12 | 0 | 0 | 0 | +107 (bílý na tahu) |
| strategy | promo-prep/f-pawn-g-promo | 12 | 0 | 0 | 0 | +38 (bílý na tahu) |
...
| game | passive-vs-active | 19 | 0 | 0 (+1 allowlist) | 0 | -412 (černý na tahu) |
...
| game | met-trap | 19 | 0 | 0 (+1 allowlist) | 0 | -291 (černý na tahu) |
| game | symmetric-opening | 20 | 0 | 0 | 0 | -38 (bílý na tahu) |
| **celkem** | 32 položek | **389** | **0** | **0** | **0** | |
...
- ztráta: **469 cp** (eval před tahem -38 → po tahu -507, z pohledu bílého)
- engine preferuje: `f1e2` Khon f1→e2 (eval -38), PV: `f1e2 e8f7 g3g4 e7e6 d1c2 g8e7`
- po zahraném tahu soupeř může: `e7e6` Khon e7×e6 (Ma) (eval +507 pro černého)
...
- ztráta: **370 cp** (eval před tahem -53 → po tahu -423, z pohledu černého)
- engine preferuje: `e6e5` Bia e6→e5 (eval -53), PV: `e6e5`
- po zahraném tahu soupeř může: `f4e6` Ma f4×e6 (Bia) (eval +423 pro bílého)
...
- engine preferuje: `b7a6` Khun b7×a6 (Ma) (eval 0), PV: `b7a6 b5a3 a6a5 a3c2 a5a4 c2e3`
...
- pozice už rozhodnutá (pomalejší cesta k výhře): eval M12 → +5887 z pohledu bílého; engine preferuje `a2f2` Rua a2→f2

#### tah 4 (černý) — `g7g6` Khun g7→g6 · data [1,6]→[2,6] — _rozhodnuto (nepočítá se)_

- pozice už rozhodnutá (volba mezi prohrávajícími tahy): eval -5887 → -M24 z pohledu černého; engine preferuje `g7f6` Khun g7→f6
...
- ztráta: **295 cp** (eval před tahem -12 → po tahu -307, z pohledu černého)
- engine preferuje: `g8e7` Ma g8→e7 (eval -12), PV: `g8e7 c2d3 e8f7 f2e3 b7b6 a3a4`
- po zahraném tahu soupeř může: `b4c5` Bia b4×c5 (Met) (eval +307 pro bílého)
...
|---|---|---:|---|---|---:|---|---:|---:|---:|---|
| game | two-knights-mate | 6 | černý | `b7a6` Khun b7×a6 (Ma) | 0 | `b7c8` Khun b7→c8 | -M1 | 9990 | +980 | `b7a6 b5a3 a6a5 a3c2 a5a4 c2e3 a4a3 e3d1` |
...
| strategy | central-push/c-pawn-knights | bílý | +38 | `c1d2` Khon c1→d2 | — | „Kůň na e7 kryje d5 — jediného centrálního pěšce černého, kterého bílý kůň z c3 už napadá. Bilance: struktura …" |
| strategy | central-push/e-pawn-kingside | bílý | -8 | `g3f4` Bia g3×f4 (Bia) | — | „Černý bere e5×f4 dřív, než pěšec padne. Bílý vezme zpět g3×f4 a bude mít, co chtěl: koně na e4 a f3, pěšce d4…" |
...
| strategy | met-attack/met-g5-support | bílý | +115 | `g1f3` Ma g1→f3 | — | „Kůň na f6. Hlídá g4 a h5 a černý si chystá figury, kterými by Met později obklíčil. Bílý ale má, co chtěl: Me…" |
| strategy | met-attack/met-against-f5 | bílý | +15 | `g1e2` Ma g1→e2 | — | „c5. Černý hledá protihru tam, kde bílá Met není: na dámském křídle. Bilance: Met stojí na f4 pevně a bez star…" |
| strategy | met-attack/met-h4-diagonal | bílý | -84 | `d3e4` Bia d3×e4 (Bia) | — | „e4! Pěšec e5 byl napadený dvakrát (f4, Ma f3) a krytý jen koněm z d7, tak jde vpřed a sám útočí — na d3 i na …" |
...
| strategy | rua-on-7th/open-a-file | bílý | +30 | `f1e2` Khon f1→e2 | — | „Khon se vrací na b7 a kryje pěšce b5. Poučení: otevřít sloupec, na kterém stojí soupeřova věž, znamená výměnu…" |
...
| strategy | rua-on-7th/b-file-rook | bílý | +61 | `c4a5` Ma c4×a5 (Bia) | — | „d5! Černý pěšce a5 neudrží — kdyby ho bránil postupem a5-a4, kůň by místo něj vzal d6. Raději napadá koně pěš…" |
| strategy | rua-on-7th/a-file-active-knights | bílý | +69 | `g1f3` Ma g1→f3 | — | „d5. Černý napadá c4 a hledá protihru v centru. Bilance: bílý má aktivního koně c3 a druhého na cestě na f3, o…" |
| strategy | promo-prep/e-file-push | bílý | -15 | `f3e4` Bia f3×e4 (Bia) | — | „Kůň na c5 — uhýbá z dosahu P+ (ten bere diagonálně všemi směry, takže na d7 by ho vzal) a zároveň P+ na e6 sá…" |
| strategy | promo-prep/d-pawn-e5-promo | bílý | -46 | `d1c2` Khun d1→c2 | — | „Kůň na d7 — kryje e5 a připravuje Khon f8-e7-d6, aby volného pěšce zablokoval natrvalo. Bilance: bílý má voln…" |
| strategy | promo-prep/b-pawn-promo | bílý | -68 | `a4b5` Bia a4×b5 (Bia) | — | „Černý bere c6×b5 — poslední výměna. Bilance: bílý teď vezme a4×b5, černý R×a1 a Khon b2 bere zpět: věže se vy…" |
...
| strategy | promo-prep/f-pawn-g-promo | bílý | +38 | `d1c2` Khun d1→c2 | — | „Khun na f7. Král kryje g6 i e6 a černý má vše pokryté. Bilance: materiál je vyrovnaný, promoce proběhla a hne…" |
...
| game | passive-vs-active | černý | -412 | `e8e7` Khun e8→e7 | — | **Útočník (bílý) vyhraje materiálem — pasivně postavené figury nekryjí e6 a Khon g7 padne** — „Kůň bere Khon. Bilance: bílý má Khon a pěšce navíc a jeho kůň stojí hluboko v soupeřově pozici, kde ho nikdo …" |
...
| game | met-trap | černý | -291 | `g8e7` Ma g8→e7 | — | **Bílý získává Met (past na c5)** — „Pěšec bere zpět a výměna končí: kůň za koně, Met za nic. Bilance: bílý má Met navíc a pěšec c5 mu k tomu drží…" |
| game | symmetric-opening | bílý | -38 | `f3f4` Bia f3→f4 | — | **Vyrovnaná pozice — symetrii rozbil c4 a černý ji správně nekopíroval** — „Met uhýbá z dostřelu věže na d6, kde stojí bezpečně a hlídá c5 i e5. Bilance po 20 tazích: materiál rovný, po…" |
...
- eval před tahem -30 → po tahu +7 (z pohledu černého); ztráta **-37 cp** → **ok**
- engine místo toho preferuje: `b6b5` Bia b6→b5, PV `b6b5 e2d3 b5c4 b3c4 c6c5 d4d5`
...
- eval před tahem +69 → po tahu +30 (z pohledu bílého); ztráta **39 cp** → **ok**
- engine místo toho preferuje: `c3c4` Bia c3→c4, PV `c3c4 c6c5 g1e2 g8f6 b1c3 b8c6`
...
- eval před tahem +61 → po tahu +46 (z pohledu bílého); ztráta **15 cp** → **ok**
- engine místo toho preferuje: `f3f4` Bia f3→f4, PV `f3f4 b8c6 g1f3 g8e7 c3c4 b5c4`
- po zahraném tahu soupeř může: `b5b4` Bia b5→b4 (eval -46 pro černého)
- tah 6 (černý) `b5c4` Bia b5×c4 (Bia): zmeškané braní **nezachyceno** (engine preferuje `b5b4` Bia b5→b4, ztráta 0 cp)
...
- eval před tahem +307 → po tahu +276 (z pohledu bílého); ztráta **31 cp** → **ok**
- engine místo toho preferuje: `b4c5` Bia b4×c5 (Met), PV `b4c5 g8e7 c2d3 e8f7 e1d2 a6a5`
- po zahraném tahu soupeř může: `b7b6` Khon b7→b6 (eval -276 pro černého)
...
- eval před tahem -46 → po tahu -46 (z pohledu bílého); ztráta **0 cp** → **ok**
- engine místo toho preferuje: `b2c3` Khon b2→c3, PV `b2c3 d5e4 f3e4 e8f7 d1c2 c7d6`
- po zahraném tahu soupeř může: `a8c8` Rua a8→c8 (eval +46 pro černého)
